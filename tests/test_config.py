...
def build_cleanup_config():
    with open(config.Config.user_config_path, 'w') as config_file:
...
def test_create_config(build_cleanup_config):
    os.remove(config.Config.user_config_path)
    test_config = config.Config()
    assert test_config == config.Config.default_config
    with open(config.Config.user_config_path, 'r') as config_file:
