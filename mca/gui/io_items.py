...
class InputItem(QtWidgets.QGraphicsItem):
    """Class to display an :class:`.Input` in the UI. This class supports all functionalities of :class:`.Input`.
    ...
    Attributes:
        ...
        connection_line: :class:`.ConnectionLine` to the :class:`.OutputItem` it is connected to. None if its not
                         connected.
    ...
    def paint(self, painter, option, widget):
        """Method to paint the the input. This method gets invoked after initialization
        and every time the input gets updated.
       """
    ...
    def mouseReleaseEvent(self, event):
        """Attaches the connection line to an :class:`.OutputItem` if the mouse gets released above one. Otherwise
        the connection line gets removed.
        """
    ...
    def update_connection_line(self):
        """Method to update its connection line according to its own position."""
        if self.connection_line:
            self.connection_line.x2 = self.scenePos().x() + 5
            self.connection_line.y2 = self.scenePos().y() + self.height / 2

    def contextMenuEvent(self, event):
        """Method that is invoked when the user right-clicks the input. Opens the context menu of the input."""
...
class OutputItem(QtWidgets.QGraphicsItem):
    """Class to display an :class:`.Output` in the UI. This class supports all functionalities of :class:`.Output`.
    ...
    def paint(self, painter, option, widget):
        """Method to paint the output. This method gets invoked after initialization
        and every time the output gets updated.
        """
        path = QtGui.QPainterPath()
        path.moveTo(0, 0)
    ...
    def mousePressEvent(self, event):
        """Method invoked when the block gets clicked. Creates a :class:`.ConnectionLine` and
        adds it to the connection line list.
        """
    ...
    def mouseReleaseEvent(self, event):
        """Attaches the last added connection line to an :class:`.InputItem` if the mouse gets released above one.
        Otherwise the last added connection line gets removed.
        """
        for i in self.scene().items(event.scenePos()):
    ...
    def update_connection_line(self):
        """Method to update all its connection lines according to its own position."""
    ...
    def disconnect(self):
        """Disconnects the :class:`.OutputItem` from all its :class:`.InputItem` s."""
    ...
    def contextMenuEvent(self, event):
        """Method that is invoked when the user right-clicks the output. Opens the context menu of the output."""
        self.menu.exec_(event.screenPos())


class ConnectionLine(QtWidgets.QGraphicsLineItem):
    """Class which represents the connection between an :class:`.Input` and an :class:`.Output`.
