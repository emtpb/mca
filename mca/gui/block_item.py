...
class BlockItem(QtWidgets.QGraphicsItem):
    ...
    Attributes:
        ...
        resize_mode (bool): Flag to indicate whether user is resizing or moving the block.
        last_point (tuple): Used when resizing the block to remember the coordinates of the last mouse movement to
                            calculate the current size.
        menu: Menu which pops up when the right mouse button is pressed.
        parameter_window: Window which carries all parameters of the block as :mod:`.parameter_widgets`
                          to manipulate the parameters.
        parameter_action: Action added to the menu which opens the :class:`.ParameterWindow` of the class.
        add_input_action: Action added to the menu which only exists when the block instance
                          is a :class:`.DynamicBlock`. A new :class:`.InputItem` is dynamically added.
        delete_input_action: Action added to the menu which only exists when the block instance
                          is a :class:`.DynamicBlock`. The last :class:`.InputItem is deleted of the input list.
        show_plot_action: Action added to the menu when the block has a function called "show". The action invokes the
                          function.
    ...
    def paint(self, painter, option, widget):
        """Method to paint the the block. This method gets invoked after initialization
        and every time the block gets updated.
        """
    ...
    def contextMenuEvent(self, event):
        """Method that is invoked when the user right-clicks the block. Opens the context menu of the block."""
        self.menu.exec_(event.screenPos())

    def delete_input(self):
        """Deletes the last input in the input list and deletes it also from the block instance.
        This method is connected to delete_input_action.
        """
    ...
    def create_new_input(self):
        """Creates a new :class:`.Input` adds it to the block instance and puts into a :class:`.InputItem`.
        This method is connected to create_input_action.
        """
    ...
    def delete(self):
        """Disconnects all its inputs and outputs and removes itself from the scene."""
    ...
    def add_existing_input(self, input):
        """Adds an existing :class:`.Input` from the block instance to a new :class:`.InputItem` and
        adds it to its input list.
        """
    ...
    def add_existing_output(self, output):
        """Adds an existing :class:`.Output` from the block instance to a new :class:`.OutputItem` and
        adds it to its output list.
        """
    ...
    def mousePressEvent(self, event):
        """Method invoked when the block gets clicked. Increases its Z value guarantee to be displayed in front of
        other colliding blocks. If the bottom right 20x20 pixels of the block is pressed the block switches to
        resize mode.
        """
    ...
    def mouseMoveEvent(self, event):
        """Method invoked when a mouse button was pressed whilst moving the mouse. If resize_mode is False
        then the block gets moved otherwise the block gets resized.
        """
        if self.resize_mode:
            if self.last_point[0] is not None and self.last_point[1] is not None:
                self.resize(self.width + event.screenPos().x() - self.last_point[0], self.height + event.screenPos().y() - self.last_point[1])
    ...
    def resize(self, width, height):
        """Resizes the block to the given width and height if they are bigger than the min_height and the min_width
        and also bigger than the space the inputs or outputs are requiring.
        ...
        if max(len(self.outputs) * (self.output_height + self.output_dist) + 5,
               len(self.inputs) * (self.input_height + self.input_dist) + 5) > height or height < self.min_height:
            return
